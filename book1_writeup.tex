...
each subject heard the same target clip in each block but with different
target-defining instructions (literal target vs.\ imagined target).
...
\begin{subfigure}[t]{0.48\textwidth}
  ...
  \caption{$300{\times}300$ correlation matrix, ordered targets-then-distractors.  The faint warm block in the upper-left is the entire effect.}
\end{subfigure}
